...
let color2grey x = 
	let z = int_of_float ( ( level x ) *. 255.0 ) in (z,z,z)
...
(* Best truc qui foire EU )
(
let rec nbpix_line input x line = 
  let (w,h) = Matrix.get_dims input in 
  (*  	if (x<=w) then*)
   	   let isdatpixelblack m mx my = (Matrix.get m mx my) = 255 in
        	if (isdatpixelblack input x line) then 1 + (nbpix_line input (x+1) line)
        	else nbpix_line input (x+1) line
(*	else 0*)

let rot input = if (nbpix_line input 01 01 > 10) then rotate input 90.
		else rotate input 180.

let rec rotate_line mat ang num = match (num) with
		|0 -> rotate_line mat ang (nbpix_line mat num (num+1))
		|n when (n<15) -> rotatee input ang
		|_ -> failwith "ah" in
   rotate_line input 01. (nbpix_line input 01 01)
*)
        ...
        for y=0 to h-1 do
           for x=0 to w-1 do
             ...
             (* t est l angle qu on fera variee -pi/2 a pi/2 *)
                let t = ref(-.pi02) in
                (* parcours de l'inertvalle d'angle *)
                 while ( !t <= pi02 ) do
                       ...
                       if droite>=0 then (* on remplis le tableau de vote *)
                         begin
                          let teta_i = int_of_float(!t*.100. +. pi02*.100.)in
                           ...
                           if !vote_max < float_of_int(matrice_de_vote.(droite).(teta_i)) then
                                  begin
                                  vote_max := float_of_int(matrice_de_vote.(droite).(teta_i));
                                  teta_max := !t;
                                  end;
                         end;
